...
volatile UINT8 accelcorrParamsUpdated = 0;
...
interrupt 38 void CANRx_vect(void)
{
  ...
  if(identifier == CAN_PARAM_MSG_ID)
    ...
    {
       if(!accelParamsUpdated) 
       {
          if(length > sizeof(AccelMsg))
                length = sizeof(AccelMsg);
          
           memcpy(&(params.Accelparams), &CANRXDSR0, length);
            accelParamsUpdated = 1;
       }
    } else if(identifier == CAN_ACCEL_CORR_MSG_ID) 
    {
     if(!accelcorrParamsUpdated) 
       {
          if(length > sizeof(AccelMsg))
                length = sizeof(AccelMsg);
          
           memcpy(&(params.Accelcorrparams), &CANRXDSR0, length);
            accelcorrParamsUpdated = 1;
       }
...
void main(void)
{
    ...
    for(;;)
    {
        ...
        if(brakeParamsUpdated && carParamsUpdated && accelcorrParamsUpdated && accelParamsUpdated)
        {
            SCITxPkt(&params, sizeof(Allparams));
            carParamsUpdated = 0;
            brakeParamsUpdated = 0;
            accelcorrParamsUpdated =0;
...
void init()
{                                    
setclk8();   // set Eclock to 8 MHZ
    SCIInit(); // set to 38400 
